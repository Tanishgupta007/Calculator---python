...
screen.maxsize(width=470, height=585)
screen.minsize(width=363, height=585)
screen.geometry('363x585')
...
def on_enterpoint(e):
    btnpoint.configure(bg='red')
def on_leavepoint(e):
    btnpoint.configure(bg='powder blue')
def on_entername(e):
    btnname.configure(bg='red')
def on_leavename(e):
    btnname.configure(bg='powder blue')
def on_enterlog(e):
...
operator = ''
#Text
expandlabel= Label(screen, text='Expand for scientific\nCalculator --->', font=('arial', 18, 'italic bold'), bg='yellow', fg='blue')
expandlabel.place(x=110, y=500)
...
btnpoint = Button(screen, bg='powder blue', text='.', font=('arial', 15, 'italic bold'), bd=15, padx=18,pady=16, command=lambda: click('.'), activebackground='yellow', activeforeground='red')
btnpoint.grid(row=4, column=0)
btnpoint.bind('<Enter>', on_enterpoint)
btnpoint.bind('<Leave>', on_leavepoint)


btnname = Button(screen, bg='powder blue', text='G', font=('arial', 15, 'italic bold'), bd=15, padx=15, pady=16, command=lambda: webbrowser.open_new('https://github.com/Tanishgupta007'), activebackground='yellow', activeforeground='red')
btnname.grid(row=5, column=4)
btnname.bind('<Enter>', on_entername)
btnname.bind('<Leave>', on_leavename)
